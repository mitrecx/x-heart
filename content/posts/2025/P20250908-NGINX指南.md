...
nginx 是一个高性能的 **HTTP 和 反向代理服务器(Reverse Proxy Server)**，也是一个 IMAP/POP3/SMTP 代理服务器。  
- 作为 HTTP 服务器，主要 处理静态资源请求。静态资源包括 HTML、CSS、JS、图片、视频等文件。  
- 作为 反向代理服务器, 主要 将客户端请求 转发到后端服务，并将后端服务器的响应返回给客户端。  
